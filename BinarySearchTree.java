...
public class BinarySearchTree<E extends Comparable<E>> {
    ...
    public boolean add(E val) {
        ...
        return size != this.size;   //Only adding successfully will change the size, make"!=" true. Otherwise it is false.
    }
    ...
    protected int depth(E val) {
    	...
    	int count = 0;
    	return findNode(this.root,val,count);
    }
    
    private int findNode(Node n, E val, int count) {
    	if(n == null) {
    		return -1;
    	}
    	...
    	if(cmp < 0) {
    		count++;
    		return findNode(n.left,val,count);
    	}
    	else if(cmp > 0) {
    		count++;
    		return findNode(n.right,val,count);
    	}
    	else {
    		return count;
    	}
    ...
    private static int findHeight(BinarySearchTree.Node n) {
    	if(n == null) {
    		return 0;
    	}
    ...
    protected static boolean isBalancedNode(BinarySearchTree<?>.Node n) {
        ...
        int height_left_children = findHeight(n.left)+1;
        int height_right_children = findHeight(n.right)+1;
        
        int difference = height_left_children - height_right_children;
    ...
    public boolean isBalanced() {
        ...
        int indicator = 0;
        return checkAll(this.root,indicator);
    }
    
    private boolean checkAll(BinarySearchTree<?>.Node n,int indicator) {
    	if(n == null) {
    		return true;
    	}
    	
    	if(isBalancedNode(n) != true) {
            indicator = -1;
            return false;
    	}
    	
    	checkAll(n.left,indicator);
    	checkAll(n.right,indicator);  
